...
def doping(Nx, Ny, Ntotal, junction_l, junction_r, Nd, N_sd, N_body):
    ...
    if dopslope_s != 0 or dopslope_d != 0:
        decay_lens = (2.0/math.sqrt(2.3)*dopslope_s)
        decay_lend = (2.0/math.sqrt(2.3)*dopslope_d)
    ...
    for iii_row in np.arange(((Nx*(t_topa+1))/Nx), ((Ntotal-Nx*t_bota)/Nx-1)):
        ...
        for iii_col in np.arange(junction_l, junction_r-1):
            ...
            if dopslope_s != 0 and dopslope_d != 0:
                ...
                Nd[i_node] = N_sd*np.exp(-((iii_col-junction_r+1)*dx/decay_lend)**2)-N_body
    ...
    ######################ABRUPT PROFILE on BOTH SIDE#########################
        ...
        if ox_pnt_flag == 1:
            ...
            Nd[(Ntotal-Nx*(t_bota+2)):(Ntotal-Nx*(t_bota+2)+junction_l)] = (N_sd-N_body)/2.0
            Nd[(Ntotal-Nx*(t_bota+2))+junction_l:(Ntotal-Nx*(t_bota+2)+1)+junction_r-1] = -N_body/2.0
            Nd[(Ntotal-Nx*(t_bota+2))+junction_r-1:(Ntotal-Nx*(t_bota+2)+Nx)] = (N_sd-N_body)/2.0
