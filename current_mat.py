...
def current_mat(mu_scatter_old, T_E, E):
    ...
    T_dummy = np.sum(T_E, 2)

    #***************************************************************************************
    #*************************************EVALUATE Iin**************************************
    #***************************************************************************************
    for i_node in np.arange(0,nu_scatter):
        I_dummy1 = fermi(((mu_scatter_old[i_node]-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0)*T_dummy[:,i_node]
        I_dummy2 = 0
        for j_node in np.arange(0,nu_scatter+2):
            I_dummy2 = I_dummy2 + fermi(((mu_scatter_old[j_node]-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.-0)*T_E[j_node, :, i_node]
    ...
    if Isc>=I_criterion:
        for i_node in np.arange(0, nu_scatter):
            IMU_dummy[i_node,i_node]=((fermi(((mu_scatter_old[i_node]+dmu-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0)-fermi(((mu_scatter_old[i_node]-dmu-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0))/(2.0*dmu))*T_dummy[:,i_node]
            for j_node in np.arange(0, nu_scatter):
                IMU[i_node,j_node]=((fermi(((mu_scatter_old[j_node]+dmu-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0)-fermi(((mu_scatter_old[j_node]-dmu-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0))/(2.0*dmu))*T_E[j_node, :,i_node]
        IMU=IMU_dummy-IMU
    ...
    while(Isc>=I_criterion):
        ...
        #****************************************************************************************
        for i_node in np.arange(0,nu_scatter):
            I_dummy1 = fermi(((mu_scatter_new[i_node]-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0)*T_dummy[:,i_node];
            I_dummy2=0
            for j_node in np.arange(0,nu_scatter+2):
                I_dummy2=I_dummy2+fermi(((mu_scatter_new[j_node]-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0)*T_E[j_node, :,i_node]
            Iin[i_node] = I_dummy1-I_dummy2
        Isc = max(abs(Iin))
        ...
        #****************************************************************************************
            for i_node in np.arange(0,nu_scatter):
                IMU_dummy[i_node,i_node]=((fermi(((mu_scatter_new[i_node]+dmu-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0)-fermi(((mu_scatter_new[i_node]-dmu-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0))/(2.0*dmu))*T_dummy[:,i_node]
                for j_node in np.arange(0,nu_scatter):
                    IMU[i_node,j_node]=((fermi(((mu_scatter_new[j_node]+dmu-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0)-fermi(((mu_scatter_new[j_node]-dmu-E)/(k_B*Temp/q)),fermi_flag,-1.0/2.0))/(2.0*dmu))*T_E[j_node, :,i_node]
            IMU=IMU_dummy-IMU
    ...
    for i_node in np.arange(0, 2):
        I_dummy1 = fermi(((mu_scatter_new(i_node+nu_scatter)-E)/(k_B*Temp/q)),fermi_flag,-1/2)*T_dummy[:,i_node+nu_scatter]
        I_dummy2=0
        for j_node in np.arange(0, nu_scatter+2):
            I_dummy2 = I_dummy2+fermi(((mu_scatter_new(j_node)-E)/(k_B*Temp/q)),fermi_flag,-1/2)*T_E[j_node, :, i_node+nu_scatter]
